...
def generate_appointment_pdf(appointment):
    try:
        ...
        story.append(Paragraph(f"Contact: {settings.HOSPITAL_CONTACT_NUMBER} | {settings.HOSPITAL_CONTACT_EMAIL}", footer_style))
        story.append(Paragraph(getattr(settings, 'HOSPITAL_LEGAL_FOOTER', f"Generated by {settings.SITE_NAME}"), footer_style))
